...
const StatsCard: React.FC<StatsCardProps> = ({ 
  ...
  if (theme === "spartan") {
    return (
      <Card className="hover:shadow-lg transition-all duration-300 border-2 hover:border-amber-400 bg-gradient-to-br from-amber-50 to-amber-100">
        ...
        <CardContent>
          <div className="text-3xl font-bold bg-gradient-to-r from-amber-600 to-red-700 bg-clip-text text-transparent">{value}</div>
          <p className="text-sm text-gray-500 italic">{description}</p>
          <ProgressBar 
            progress={progress}
            className="mt-3"
            colorClass={`bg-gradient-to-r from-amber-500 to-red-600`}
          />
        </CardContent>
      </Card>
    );
